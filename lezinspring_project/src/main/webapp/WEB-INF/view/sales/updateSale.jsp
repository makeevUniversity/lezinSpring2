...
<script>
    ...
    app.controller('updateSaleController', function ($scope, $http) {
        var params = window
            .location
            .search
            .replace('?', '')
        ...
        $http({method: 'GET', url: '/rest/findByIdSale', params: {'id': params['id']}}).then(
            function (response) {
                $scope.sale = response.data;
                $scope.sale.dateOfDelivery = new Date(response.data.dateOfDelivery);
                $scope.sale.orderDate = new Date(response.data.orderDate);
            },
            function (response) {
        ...
        $http.get('/rest/findAllSellers').then(
            function (response) {
                ...
                for (var i = 0; i < list.length; i++) {
                    if (list[i].id == $scope.sale.seller.id) {
                        $scope.sale.seller=list[i];
                    }
                }
            },
            function (response) {
        ...
        $http.get('/rest/findAllProducts').then(
            function (response) {
                ...
                var list=response.data;
                for (var i = 0; i < list.length; i++) {
                    if (list[i].id == $scope.sale.product.id) {
                        $scope.sale.product=list[i];
                    }
                }
            },
            function (response) {
        ...
        $http.get('/rest/findAllBuyers').then(
            function (response) {
                ...
                var list=response.data;
                for (var i = 0; i < list.length; i++) {
                    if (list[i].id == $scope.sale.buyer.id) {
                        $scope.sale.buyer=list[i];
                    }
                }
            },
            function (response) {
        ...
        $scope.submit = function () {
            $http.post('/rest/updateSale', $scope.sale).then(
                function (response) {
...
<div class="col-md-4" ng-app="myAppAdd" ng-controller="updateSaleController">
