...
<script>
    ...
    app.controller('updateDiscountController', function ($scope, $http) {
        var params = window
            .location
            .search
            .replace('?', '')
        ...
        $http({method: 'GET', url: '/rest/findByIdDiscount', params: {'id': params['id']}}).then(
            function (response) {
        ...
        $http.get('/rest/findAllSellers').then(
            function (response) {
                ...
                for (var i = 0; i < list.length; i++) {
                    if (list[i].id == $scope.discount.seller.id) {
                        $scope.discount.seller=list[i];
                    }
                }
            },
            function (response) {
        ...
        $http.get('/rest/findAllProducts').then(
            function (response) {
                ...
                var list=response.data;
                for (var i = 0; i < list.length; i++) {
                    if (list[i].id == $scope.discount.product.id) {
                        $scope.discount.product=list[i];
                    }
                }
            },
            function (response) {
        ...
        $scope.submit = function () {
            $http.post('/rest/updateDiscount', $scope.discount).then(
                function (response) {
                    document.location.href = "/rest/discountList.html";
                },
                function (response) {
                    alert("Error:" + response.status);
                });
...
<div class="col-md-4" ng-app="myAppAdd" ng-controller="updateDiscountController">
