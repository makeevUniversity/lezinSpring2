...
<script>
    ...
    app.controller('addProductController', function ($scope, $http, $location) {
        ...
        $http({method: 'GET', url: '/rest/findByIdProduct', params: {'id': params['id']}}).then(
            function (response) {
        ...
        $scope.submit = function () {
            $http.post('/rest/updateProduct', $scope.product).then(
                function (response) {
                    document.location.href = "/rest/productList.html";
                },
                function (response) {
                    alert("Error:" + response.status);
                });
